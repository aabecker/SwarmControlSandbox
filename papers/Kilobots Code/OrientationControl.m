...
function OrientationControl()
    clear all
    global q goalAngle goalAngleRad goalChanged
    %% Define webcam
    webcamShot = false;
    ...
    %             success=true;
        ...
        drawTime = [drawTime;newDot];
            ...
            if ~VarCont
                ...
                elseif ObjectOrientation>goalAngleRad+(5*pi/180)||ObjectOrientation<goalAngleRad-(5*pi/180)
                    ...
                    %% If goal just changed be different
                    if goalChanged
                        %% go to near next goal
                        if dist2points(M(1),M(2),currgoalX - cos(ObjectOrientation+pi/2)* ObjectLength/5,currgoalY + sin(ObjectOrientation+pi/2)* ObjectLength/5)<dist2points(M(1),M(2),currgoalX + cos(ObjectOrientation+pi/2)* ObjectLength/2.3,currgoalY - sin(ObjectOrientation+pi/2)* ObjectLength/2.3)
                            currgoalX = currgoalX - cos(ObjectOrientation+pi/2)* ObjectLength/3.5;
                            currgoalY = currgoalY + sin(ObjectOrientation+pi/2)* ObjectLength/3.5;
                        else
                            currgoalX = currgoalX + cos(ObjectOrientation+pi/2)* ObjectLength/2.3;
                            currgoalY = currgoalY - sin(ObjectOrientation+pi/2)* ObjectLength/2.3;
                        end
%                         %% go to center
%                         if dist2points(M(1),M(2),ObjectCentroidX - cos(ObjectOrientation+pi/2)* ObjectLength/2.3,ObjectCentroidY + sin(ObjectOrientation+pi/2)* ObjectLength/2.3)<dist2points(M(1),M(2),ObjectCentroidX + cos(ObjectOrientation+pi/2)* ObjectLength/2.3,ObjectCentroidY - sin(ObjectOrientation+pi/2)* ObjectLength/2.3)
%                             currgoalX = ObjectCentroidX - cos(ObjectOrientation+pi/2)* ObjectLength/2.3;
%                             currgoalY = ObjectCentroidY + sin(ObjectOrientation+pi/2)* ObjectLength/2.3;
%                         else
%                             currgoalX = ObjectCentroidX + cos(ObjectOrientation+pi/2)* ObjectLength/2.3;
%                             currgoalY = ObjectCentroidY - sin(ObjectOrientation+pi/2)* ObjectLength/2.3;
%                         end
                        %% Check if near goal, if so, start going towards point on bar
                        if (M(1) < currgoalX + epsilon && M(1) > currgoalX - epsilon) && (M(2) < currgoalY + epsilon && M(2) > currgoalY - epsilon)
                            goalChanged = 0;
                        end
                    end
                %% Go to closest corner if object is oriented correctly   
                else
                    for i = 1:size(corners)
                        dist = dist2points(corners(i,1),corners(i,2),M(1)/scale,M(2)/scale);
                        if minDis > dist
            ...
            if flowDebug
                ...
                hq=quiver(X*scale,Y*scale,DX,DY,'color','cyan');
            end
