...
\begin{overpic}[width =0.4\columnwidth]{AutoControlVaryN.pdf}
\end{overpic}
\begin{overpic}[width =0.5\columnwidth]{PolicyIter.eps}
\end{overpic}
\vspace{-1em}
\caption{\label{fig:NumRobotsVSPolicy}Completion-time results using the automatic controller from Alg.~\ref{alg:BlockPushing} for different numbers of robots.  Each bar is labelled with the number of trials.
%\vspace{-2em}
}
\end{figure*}
