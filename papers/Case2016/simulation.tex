...
Each task uses a different algorithm to select the swarm's goal position $[goal_x,goal_y]^T$. The goal is to stay at the goal position, so the desired velocity at goal position is 0.
...
Because few robots remain,  the force is significantly decreased and torque is not maximized.
 In our simulation, the swarm applies torque until the swarm's mean position is beyond the object.  The swarm will regather in a corner at this point before returning to torque control, a time-consuming task. 
 The key parameter of interest for a hinged door of length $L$ is $C$, the position along the door where the mean of the swarm will push.  The swarm is directed toward these goal calculations:
...
Where $O_{\theta}$ is the object's orientation.
